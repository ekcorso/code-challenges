...
struct Stat {
    ...
    init(scores: [Int]) {
        ...
        self.range = format(rangeInSec)
        self.average = format(averageInSec)
        self.median = format(medianInSec)
    }
}

func format(_ stat: Int) -> String {
    let hours = stat / 3600
...
func getTeamStats(_ statListStr: String) -> String {
    ...
    let teamStats = Stat(scores: scoresInSeconds)
    
    return "Range: \(teamStats.range) Average: \(teamStats.average) Median: \(teamStats.median)"
}
